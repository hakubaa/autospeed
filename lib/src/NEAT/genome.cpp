...
Genome::Genome(int id, std::ifstream &iFile) {
	...
	// read one line from the file, it's repeated in the loop
...
bool Genome::mutate_addSensorToOutputGene(std::vector<Innovation*> &innovs,double &curinnov) {

	// Collect all output nodes
	std::vector<NNode*> outputs;
	std::vector<NNode*> sensors;

	for(std::vector<NNode*>::iterator node = nodes.begin(); node != nodes.end(); ++node) {
		if ((*node)->gen_node_label == OUTPUT)
			outputs.push_back(*node);
		else if ((*node)->get_type() == SENSOR)
			sensors.push_back(*node);
	}
	...
	// Select randomly one output node
	NNode* outputNode = outputs[randint(0, outputs.size()-1)];

	//for(std::vector<NNode*>::iterator outputNode = outputs.begin(); outputNode != outputs.end(); ++outputNode) {

	for (std::vector<Link*>::iterator link = (outputNode)->incoming.begin(); 
		link != (outputNode)->incoming.end(); ++link) {

		if ((*link)->in_node->get_type() == SENSOR) {
			// remove this sensor from sensors vector
			std::vector<NNode*>::iterator sensor = sensors.begin();
			while ((*sensor)->node_id != (*link)->in_node->node_id && sensor != sensors.end())
				sensor++; 
	...
	NNode* sensor = sensors[randint(0, sensors.size()-1)];

	//Check to see if this innovation already occured in the population
	std::vector<Innovation*>::iterator theinnov = innovs.begin();
	...
	while(!done) {
		...
		if (theinnov == innovs.end()) {
			...
			//Choose a random trait
			int traitnum = traitnum=randint(0,(traits.size())-1);
			std::vector<Trait*>::iterator thetrait = traits.begin();
			...
			//Create the new gene
			newgene = new Gene(thetrait[traitnum], newweight, 
				sensor, outputNode, 0, curinnov, newweight);

			//Add the innovation
			innovs.push_back(new Innovation(sensor->node_id, (outputNode)->node_id, curinnov, newweight, traitnum));

			curinnov = curinnov + 1.0;

			done = true;

		} else if (((*theinnov)->innovation_type == NEWLINK) && ((*theinnov)->node_in_id == (sensor->node_id)) &&
			((*theinnov)->node_out_id == ((outputNode)->node_id)) && ((*theinnov)->recur_flag == false)) {

				std::vector<Trait*>::iterator thetrait = traits.begin();

				//Create new gene
				newgene = new Gene(((thetrait[(*theinnov)->new_traitnum])), (*theinnov)->new_weight, 
					sensor, outputNode, 0, (*theinnov)->innovation_num1, 0);
