...
double NNode::activateKuba(int signalId, int t) {

	// for(int i = 0; i < t; i++)
	// 	std::cout << " ";

	//std::cout << node_id << ") activateKuba(" << signalId << ")" << std::endl;

	if (type == SENSOR) {
		// for(int i = 0; i < t; i++)
		// 	std::cout << " ";
		//std::cout << node_id << ") activation: " << get_active_out() << std::endl;
		return get_active_out();
	}
	...
	double activsum = 0;
	for(std::vector<Link*>::iterator link = incoming.begin(); link != incoming.end(); ++link) 
		activsum += (*link)->weight * (*link)->in_node->activateKuba(signalId, t + 3);

	// Identity activation function
	activation = NEAT::fsigmoid(activsum,4.924273,2.4621365);
	lastSignal = activation;

	// for(int i = 0; i < t; i++)
	// 	std::cout << " ";

	//std::cout << node_id << ") activation: " << activation << std::endl;
