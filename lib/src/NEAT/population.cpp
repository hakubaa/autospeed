...
Population::Population(const char *filename) {
	...
	if (!iFile) {
		std::stringstream s;
		s << "Population::Population(): Can't open genomes file for input";
		msg.push(s.str());
...
bool Population::print_to_file_by_species(char *filename) {
  ...
  if (!outFile) {
  	std::stringstream s;
  	s <<"Population::print_to_file_by_species(): Can't open "<<filename<<" for output"<<std::endl;
  	msg.push(s.str());
...
bool Population::epoch(int generation) {
	...
	*/
	...
	std::stringstream s;
	s <<"Number of Species: "<<num_species<<std::endl;
	msg.push(s.str());
	s.str(std::string()); // clear stream
	s <<"compat_thresh: "<<compat_threshold<<std::endl;
	msg.push(s.str());
	...
	s.str(std::string()); // clear stream
	s <<"Generation "<<generation<<": "<<"overall_average = "<<overall_average<<std::endl;
	msg.push(s.str());
	...
	for(curspecies=sorted_species.begin();curspecies!=sorted_species.end();++curspecies) {
		...
		s.str(std::string());
		s <<"orig fitness of Species"<<(*curspecies)->id<<"(Size "<<(*curspecies)->organisms.size()<<"): "<<(*((*curspecies)->organisms).begin())->orig_fitness<<" last improved "<<((*curspecies)->age-(*curspecies)->age_of_last_improvement)<<std::endl;
		msg.push(s.str());
	...
	if (((*(((*curspecies)->organisms).begin()))->orig_fitness)>
		highest_fitness) {
			...
			s.str(std::string());
			s<<"NEW POPULATION RECORD FITNESS: "<<highest_fitness<<std::endl;
			msg.push(s.str());
		...
		s.str(std::string());
		s << highest_last_changed<<" generations since last population fitness record: "<<highest_fitness<<std::endl;
		msg.push(s.str());
