...
bool Network::outputsoff() {
	...
	for(curnode=outputs.begin();curnode!=outputs.end();++curnode) {
		if (((*curnode)->activation_count)==0 && (*curnode)->incoming.size() != 0) return true;
	}
...
bool Network::activateKuba() {
	for(std::vector<NNode*>::iterator node = outputs.begin(); node != outputs.end(); ++node) 
		(*node)->activateKuba(signalId++);
	return true;
}
